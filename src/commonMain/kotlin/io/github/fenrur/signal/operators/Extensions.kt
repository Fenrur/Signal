 ...
 */
fun <S, R> Signal<S>.map(transform: (S) -> R): Signal<R> =
    MappedSignal(this, transform)
 ...
 */
fun <T> Signal<T>.pairwise(): Signal<Pair<T, T>> =
    PairwiseSignal(this)
...
fun <A, B, R> combine(
    ...
    CombinedSignal2(sa, sb, transform)
...
fun <A, B, C, R> combine(
    ...
    CombinedSignal3(sa, sb, sc, transform)
...
fun <A, B, C, D, R> combine(
    ...
    CombinedSignal4(sa, sb, sc, sd, transform)
...
fun <A, B, C, D, E, R> combine(
    ...
    CombinedSignal5(sa, sb, sc, sd, se, transform)
...
fun <A, B, C, D, E, F, R> combine(
    ...
    CombinedSignal6(sa, sb, sc, sd, se, sf, transform)
...
fun <A, B, C> zip(
    ...
    combine(sa, sb, sc) { a, b, c -> Triple(a, b, c) }
 ...
 */
fun <T> List<Signal<T>>.combineAll(): Signal<List<T>> =
    combineAll(*toTypedArray())
