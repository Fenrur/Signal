...
/**
 * Default implementation of [io.github.fenrur.signal.BindableMutableSignal] with glitch-free semantics.
 *
 * Implements [io.github.fenrur.signal.impl.ComputedSignalNode] to participate in the dependency graph.
...
class DefaultBindableMutableSignal<T>(
    initial: io.github.fenrur.signal.MutableSignal<T>? = null,
    private val takeOwnership: Boolean = false
) : io.github.fenrur.signal.BindableMutableSignal<T>, io.github.fenrur.signal.impl.ComputedSignalNode {

    private data class BindingData<T>(
        val signal: io.github.fenrur.signal.MutableSignal<T>,
    ...
    private fun bindToInternal(newSignal: io.github.fenrur.signal.MutableSignal<T>, notifyListeners: Boolean) {
        if (isClosed) return
    ...
    override fun bindTo(newSignal: io.github.fenrur.signal.MutableSignal<T>) {
        bindToInternal(newSignal, notifyListeners = true)
    }

    override fun currentSignal(): io.github.fenrur.signal.MutableSignal<T>? = bindingData.load()?.signal
