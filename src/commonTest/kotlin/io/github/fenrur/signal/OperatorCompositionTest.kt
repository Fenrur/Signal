...
class OperatorCompositionTest {
    ...
    fun `filter-map-distinctUntilChangedBy chain works correctly`() {
        ...
        val source = _root_ide_package_.io.github.fenrur.signal.impl.DefaultMutableSignal(Item(0, "init"))
        val chain = source
            .filter { it.id > 0 }
    ...
    fun `combine-map-filter chain works correctly`() {
        ...
        val chain = _root_ide_package_.io.github.fenrur.signal.operators.combine(a, b) { x, y -> x + y }
            .map { it * 2 }
            .filter { it > 20 }
    ...
    fun `flatMap with inner map chain works correctly`() {
        ...
        val inner1 = _root_ide_package_.io.github.fenrur.signal.impl.DefaultMutableSignal(100)
        val inner2 = _root_ide_package_.io.github.fenrur.signal.impl.DefaultMutableSignal(200)
    ...
    fun `nested batch through combined signals`() {
        ...
        val ab = _root_ide_package_.io.github.fenrur.signal.operators.combine(a, b) { x, y -> x + y }
        val abc = _root_ide_package_.io.github.fenrur.signal.operators.combine(ab, c) { xy, z -> xy + z }
    ...
    fun `error in combine chain propagates correctly`() {
        ...
        val chain = _root_ide_package_.io.github.fenrur.signal.operators.combine(a, b) { x, y ->
    ...
    fun `diamond pattern with different operators on each branch`() {
        ...
        val combined = _root_ide_package_.io.github.fenrur.signal.operators.combine(mapped, filtered) { m, f -> m + f }
    ...
    fun `multiple paths converging with scan`() {
        ...
        val combined = _root_ide_package_.io.github.fenrur.signal.operators.combine(aDouble, bTriple) { x, y -> x + y }
