...
class StressTest {
    ...
    fun `wide graph - combine many signals`() {
        val signals = List(20) { i -> _root_ide_package_.io.github.fenrur.signal.impl.DefaultMutableSignal(i) }
    ...
    fun `large batch with combined signals`() {
        val signals = List(10) { i -> _root_ide_package_.io.github.fenrur.signal.impl.DefaultMutableSignal(i) }
        val combined = signals.combineAll()
    ...
    fun `create and dispose many signals`() {
        repeat(1000) { iteration ->
            val source = _root_ide_package_.io.github.fenrur.signal.impl.DefaultMutableSignal(iteration)
            val mapped = source.map { it * 2 }
